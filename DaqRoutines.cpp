											...
											//flag to aid in scaling
{
	...
	while ( (deviceIndex < devCount) && !deviceFound )
	{
		...
		switch(devProps.deviceType) {
			case DaqBook2000:
				switch(subType) {
					case DaqSubTypeDaqBook2000A: deviceMask = (DWORD)pow(2, devProps.deviceType); break;
					case DaqSubTypeDaqBook2000E: deviceMask = (DWORD)pow(2, devProps.deviceType); break;
					case DaqSubTypeDaqBook2001 : deviceMask = (DWORD)pow(2, 26						); break;
					case DaqSubTypeDaqBook2005 : deviceMask = (DWORD)pow(2, 27						); break;
					case DaqSubTypeDaqBook2020 : deviceMask = (DWORD)pow(2, 28						); break;
					default							: deviceMask = (DWORD)pow(2, devProps.deviceType); break;
				} break;
			case DaqBoard1000 : deviceMask = (DWORD)pow(2, 24); break;
			case DaqBoard1005 : deviceMask = (DWORD)pow(2, 25); break;
			case DaqScan2000  : deviceMask = (DWORD)pow(2, 11); break;
			case DaqLab2000   : deviceMask = (DWORD)pow(2, 12); break;	
			case DaqBoard500  : deviceMask = (DWORD)pow(2, 14); break;
			case DaqBoard505  : deviceMask = (DWORD)pow(2, 15); break;
			default:
				if ( (DWORD)pow(2,devProps.deviceType) < 31 ) {
					capabilityMask &= 0;
					deviceFound = FALSE;
				} else {

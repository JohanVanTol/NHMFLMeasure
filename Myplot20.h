...
//		int GetDataColumn(int i) const;
        int SetDataColumns(int ax, int* Trace, int nTrace);
        int GetDataColumn(int i, int j=0) const;
		...
		int TickSize;
        void SetnTraces(int Ax, int nt);
        int GetnTraces(int Ax) const;
        void ResetTraces();
        int AddTrace(int trac, int col);
	...
	private :
        ...
        int LabelSize;
		int TitleSize;
		int LeftTracePenWidth;
		int RightTracePenWidth;
		int AxisPenWidth;
		...
		int **DataColumn; // Contains the associated Data columns with
        int nXTraces;
        int nLeftTraces;
        int nRightTraces;
};

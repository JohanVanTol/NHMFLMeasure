...
#include "mydata.h"
#include "GPIB_NatlInstrum.h"        // Declarations of the IEEEdevice classes
...
static void (__stdcall *PEnableLocal)(int boardID, Addr4882_t * addrlist);
...
bool LoadGPIBDll(void)
{
   ...
   PEnableLocal    = (void (__stdcall *)(int, Addr4882_t *))GetProcAddress(Gpib32Lib, (LPCSTR)"EnableLocal");
...
int GPIBInterfaceClear(int _gpib)
{
    (*PSendIFC)(_gpib);
...
int AR100W1000A::Test()
{
	...
	if (status != 0)
	{
		sprintf(ErrorMessage,"IEEE error talking to AR100W1000A at address %d", address);
	}

    status = GPIBReceive(address, response, MaxResponse -1);
 	if (status != 0)
	{
		sprintf(ErrorMessage,"IEEE error listening to AR100W1000A at address %d", address);
        ErrorCode = status;
        return ErrorCode;
	}
        // now test for correct response:  ROHDE&SCHWARZ,SMY02,826045/013,2.02

    if (strncmp(response,"Amplifier",9) !=0)
	{
		sprintf(ErrorMessage,"Incorrect ID for AR100W1000A at address %d", address);
        ErrorCode = 200;
	}
